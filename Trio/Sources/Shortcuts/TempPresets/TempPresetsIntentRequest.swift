...
final class TempPresetsIntentRequest: BaseIntentsRequest {
    ...
    @MainActor func schedulePresetTempTarget(_ preset: TempPreset, at fireAt: Date) async -> Bool {
        guard let details = await fetchPresetTempTargetDetails(preset) else {
            debug(
                .default,
                "\(DebuggingIdentifiers.failed) Preset not found or incomplete for scheduling: \(preset.name)"
            )
            return false
        }
        return await ScheduledTempTargetHelper.enact(
            name: details.name,
            targetMgdl: details.target,
    ...
    /// Resolves a preset's stored fields on a background context, extracting plain values
    /// rather than handing back an `NSManagedObject` for use off that context.
    private func fetchPresetTempTargetDetails(_ preset: TempPreset) async
        -> (name: String, target: Decimal, duration: Decimal, halfBasalTarget: Decimal?)?
    {
        let context = CoreDataStack.shared.newTaskContext()
        context.name = "fetchPresetTempTargetDetails"
        ...
        return await context.perform {
            ...
            guard let row = try? context.fetch(fetchRequest).first,
                  let target = row.target?.decimalValue,
                  let duration = row.duration?.decimalValue
            else { return nil }
