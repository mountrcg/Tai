    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            MainChartCanvas(
                ...
                showCobIobChart: showCobIobChart
            )
            .equatable()
        ...
        .onChange(of: scrollPosition) {
            updateRenderWindow()
            if !state.isChartHistoryExpanded,
               scrollPosition.timeIntervalSince(state.startMarker) < MainChartHelper.Config.historyBackfillLeadSeconds
            {
                state.expandChartHistory()
            }
        }
    ...
    @ViewBuilder private var selectionOverlay: some View {
        if let selectedGlucose, let selectionDate = selectedGlucose.date {
            ...
            if x >= 0, x <= viewportWidth {
                ...
                if let selectedCOBValue, showCobIobChart {
                ...
                if let selectedIOBValue, showCobIobChart {
...
extension MainChartCanvas {
    var mainChart: some View {
        ...
        // With the COB/IOB pane hidden this is the bottom pane, so it carries the hour labels.
...
extension MainChartCanvas: Equatable {
    static func == (lhs: MainChartCanvas, rhs: MainChartCanvas) -> Bool {
        lhs.state === rhs.state &&
            ...
            lhs.showCobIobChart == rhs.showCobIobChart
    }
}
